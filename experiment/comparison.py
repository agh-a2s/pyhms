...
def evaluator_sea_2(config, eval_limit, dim_number, function_problem, random_state):
...
def evaluator_cma_2(config, eval_limit, dim_number, function_problem, random_state):
    gsc = singular_problem_eval_limit_reached(limit=eval_limit)
    sprout = SproutMechanism(NBC_Generator(config["nbc_cut"], config["nbc_trunc"]), 
                                [NBC_FarEnough(config["nbc_far"], 2), DemeLimit(1)], 
                                [LevelLimit(config["level_limit"])])
...
if __name__ == '__main__':
    ...
    variants = {'sea_2': evaluator_sea_2, 'cma_2': evaluator_cma_2}
    test_sets = {0: [1,2,3,4,5], 1: [6,7,8,9], 2: [10,11,12,13,14], 3: [15,16,17,18,19], 4: [20,21,22,23,24]}

    with open("results/smac_results.csv", "r") as csv_file:
    ...
    def test_config(config):
        ...
        for j in test_sets[hms_config[3]]:
            ...
            result_tmp = []
            for k in range(100):
                test_problem = EvalCutoffProblem(FunctionProblem(bbob_fun, maximize=False), (evaluation_factor-ela_evaluation_factor)*dim, opt)
                tree = evaluator(hms_config[4], (evaluation_factor-ela_evaluation_factor)*dim, dim, test_problem, k+1)
                if len([o.fitness for o in tree.optima]) > 0:
                    fit = get_historic_best(tree) - opt
                else:
                    fit = 2147483647.0
            ...
            with open('backup_results.csv', 'a') as f:
                ...
                writer.writerow((result_tmp, dim, j, hms_config[4], np.mean(result_tmp)))
                print((dim, j, np.mean(result_tmp), hms_config[4]))
        return (result, dim, hms_config[4])

    hms_param_results = []
    for config in configs:
        hms_param_results.append(test_config(config))

    with open('hms_conf_evaluated.pkl', 'wb') as pickle_file:
