...
def four_funnels(x) -> float:
    def funnel(x, center) -> float:
        return np.sum((x - center) ** 2)

    return np.min(
        [
            funnel(x, center)
            for center in [np.array((-5.0, -5.0)), np.array((5.0, 5.0)), np.array((-5.0, 5.0)), np.array((5.0, -5.0))]
        ]
    )
...
class TestSprout(unittest.TestCase):
    def setUp(self):
        ...
        self.minimize_problem = FunctionProblem(lambda x: four_funnels(x), bounds=FOUR_FUNNEL_BOUNDS, maximize=False)
        self.maximize_problem = FunctionProblem(lambda x: -four_funnels(x), bounds=FOUR_FUNNEL_BOUNDS, maximize=True)
        self.initial_population_min = [
            Individual(np.array(genome[i]), self.minimize_problem, self.minimize_problem.evaluate(genome[i]))
            for i in range(len(genome))
        ]
        self.initial_population_max = [
            Individual(np.array(genome[i]), self.maximize_problem, self.maximize_problem.evaluate(genome[i]))
            for i in range(len(genome))
        ]
    ...
    def test_simple_sprout_candidates(self):
        ...
        self.assertCountEqual(first_generated[tree.root].individuals[0].genome, self.initial_population_min[0].genome)
        ...
        self.assertCountEqual(first_candidates[tree.root].individuals[0].genome, self.initial_population_min[0].genome)
    ...
    def test_nbc_sprout_candidates(self):
        ...
        self.assertCountEqual(
            np.array([ind.genome for ind in first_generated[tree.root].individuals]).flatten(),
            np.array([self.initial_population_min[i].genome for i in [0, 2, 6, 18]]).flatten(),
        )
        first_candidates = sprout.apply_deme_filters(first_generated, tree)
        self.assertCountEqual(
            np.array([ind.genome for ind in first_candidates[tree.root].individuals]).flatten(),
            self.initial_population_min[0].genome,
        )
        ...
        second_candidates = sprout.apply_deme_filters(second_generated, tree)
        self.assertCountEqual(
            np.array([ind.genome for ind in second_generated[tree.root].individuals]).flatten(),
            self.initial_population_min[2].genome,
        )
        ...
        self.assertCountEqual(
            np.array([ind.genome for ind in third_candidates[tree.root].individuals]).flatten(),
            self.initial_population_min[6].genome,
        )
    ...
    def test_simple_sprout_candidates_maximization(self):
        ...
        first_generated = sprout.candidates_generator(tree)
        self.assertCountEqual(first_generated[tree.root].individuals[0].genome, self.initial_population_max[0].genome)
        first_candidates = sprout.apply_deme_filters(first_generated, tree)
        first_candidates = sprout.apply_tree_filters(first_candidates, tree)
        self.assertCountEqual(first_candidates[tree.root].individuals[0].genome, self.initial_population_max[0].genome)
    ...
    def test_nbc_sprout_candidates_maximization(self):
        ...
        self.assertCountEqual(
            np.array([ind.genome for ind in first_generated[tree.root].individuals]).flatten(),
            np.array([self.initial_population_max[i].genome for i in [0, 2, 6, 18]]).flatten(),
        )
        first_candidates = sprout.apply_deme_filters(first_generated, tree)
        self.assertCountEqual(
            np.array([ind.genome for ind in first_candidates[tree.root].individuals]).flatten(),
            self.initial_population_max[0].genome,
        )
        ...
        self.assertCountEqual(
            np.array([ind.genome for ind in second_generated[tree.root].individuals]).flatten(),
            self.initial_population_max[2].genome,
        )
        ...
        self.assertCountEqual(
            np.array([ind.genome for ind in third_candidates[tree.root].individuals]).flatten(),
            self.initial_population_max[6].genome,
        )
        third_candidates = sprout.apply_tree_filters(third_generated, tree)
        self.assertEqual(len(third_candidates[tree.root].individuals), 0)
